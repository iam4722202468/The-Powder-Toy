...
//#TPT-Directive ElementClass Element_NAND PT_NAND 181
Element_NAND::Element_NAND()
{
	...
	HeatConduct = 251;
	Description = "Logical NAND gate. Use tmp to set delay. In: PSCN, Out: NSCN";
	...
	Update = &Element_NAND::update;
	
}

//#TPT-Directive ElementHeader Element_NAND static int update(UPDATE_FUNC_ARGS)
int Element_NAND::update(UPDATE_FUNC_ARGS)
{
	int r, rx, ry, nscnpos;
	bool sprkcunt = false;
	if (!parts[i].tmp)
